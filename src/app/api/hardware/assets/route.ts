  ...
  try {
    ...
    if (deviceId) {
      ...
      const { data: mediaFiles, error: mediaError } = await supabase
        .from('media_files')
        .select(
          'media_id, file_url, file_type, created_at, display_order, thumbnail_urls'
        )
      ...
      const formattedFiles = (mediaFiles || []).map((file) => ({
        ...
        file_url: file.file_url.startsWith('http')
          ? file.file_url
          : supabase.storage.from('vending-media').getPublicUrl(file.file_url).data
              .publicUrl,
      }));
    ...
    return NextResponse.json(
      {
        error:
          'No device associated with this user. Please associate a device with your account.',
      },
      { status: 404 }
    );
    ...
    //   if (result.device.id === 'admin-device-001') {
...
async function handleFileUpload(req: NextRequest): Promise<Response> {
  try {
    ...
    const { data: deviceData, error: deviceError } = await supabase
      ...
      .select('device_id')
      .eq('device_id', deviceId)
      .eq('user_client_id', 'adab5a8c-76dc-421a-8ade-059671c9f878')
      .single();

    console.log('Device data:', deviceData, 'Error:', deviceError);
    ...
    try {
      ...
      if (dbError) {
        ...
        return NextResponse.json(
          { error: 'Failed to save file reference: ' + dbError.message },
          { status: 500 }
        );
      }
