...
export default function MediaUploadComponent() {
  ...
  const simulateUpload = (file: File, setFileState: React.Dispatch<React.SetStateAction<FileWithPreview | null>>) => {
  ...
  const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      simulateUpload(file, (fileData) => setCompanyLogo(fileData))
    }
  }
  ...
  const handleVideoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (file) {
      simulateUpload(file, (fileData) => setVideoAd(fileData))
    }
  }
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      const newImages = Array.from(files).slice(0, 3 - companyImages.length)
      ...
      newImages.forEach((file) => {
        ...
        setCompanyImages((prev) => [...prev, fileData])
        ...
        const interval = setInterval(() => {
          setCompanyImages((prev) => {
            ...
            if (index !== -1) {
              if (updatedImages[index].progress >= 100) {
                clearInterval(interval)
                updatedImages[index] = {
                  ...updatedImages[index],
                  status: "complete",
                ...
                updatedImages[index] = {
                  ...updatedImages[index],
                  progress: updatedImages[index].progress + 10,
                }
              }
            }
