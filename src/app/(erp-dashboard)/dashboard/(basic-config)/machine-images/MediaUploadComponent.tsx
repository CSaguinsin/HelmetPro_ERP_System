...
export default function MediaUploadComponent({
    ...
    try {
      ...
      if (type === 'logo') setCompanyLogo((prev) => ({ ...prev!, status: 'complete' }));
        ...
        setCompanyImages((prev) =>
          prev.map((img) =>
            img.id === fileData.id ? { ...img, status: 'complete' } : img
          )
        );
    } catch (error) {
  ...
  const uploadProgress = totalUploads
  ...
  const handleSaveAndContinue = async () => {
    try {
      ...
      //     description:
      //       'Please upload all required media types: logo, video, and at least one image.',
      //     variant: 'destructive',
      //   });
      //   return;
      // }
    ...
    const fetchAssets = async () => {
      try {
        ...
        const videoFile = mediaFiles.find(
          (file: MediaFile) => file.file_type === 'video'
        );
        const imageFiles = mediaFiles.filter(
          (file: MediaFile) => file.file_type === 'image'
        );
          ...
          <div className="container mx-auto py-6 px-4 sm:px-6 lg:px-8">
              ...
              <div>
                ...
                {deviceDetails && (
                  <div className="flex items-center gap-2 mt-1 text-gray-600 dark:text-gray-300">
                    <span>
                      Device: <span className="font-medium">{deviceDetails.name}</span>
                    </span>
                    <Badge
                      variant="default"
                      className={
                        deviceDetails.status === 'Enable' ||
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <Card className="bg-white dark:bg-gray-800 shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                    <CardHeader className="pb-2">
                      ...
                      <CardDescription>
                        Upload up to 3 showcase images (PNG or JPG)
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-3 gap-2">
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction onClick={() => removeImage(index)}>
                                    Remove
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                        ...
                        {Array.from({ length: 3 - companyImages.length }).map(
                          (_, index) => (
                            <div
                              key={`empty-${index}`}
              ...
              <TabsContent value="images">
                <Card className="bg-white dark:bg-gray-800 shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Upload up to 3 showcase images for your company
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                  ...
                  <CardFooter className="flex justify-between">
                    <p className="text-sm text-gray-500">
                      {companyImages.length}/3 images uploaded
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
            ...
            <div className="flex justify-end gap-4 mt-8">
              ...
              <Button
                ...
                disabled={
                  totalUploads === 0 || completedUploads !== totalUploads || isSaving
                }
              >
                {isSaving ? (
                  <>
      ...
      <CardContent>
        <div className="flex flex-col items-center">
          ...
          {!fileData ? (
            ...
            <div
              className={`relative w-full rounded-lg overflow-hidden ${expanded ? 'h-48' : 'aspect-video'}`}
            >
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Are you sure you want to remove this {title.toLowerCase()}? This
                      action cannot be undone.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
