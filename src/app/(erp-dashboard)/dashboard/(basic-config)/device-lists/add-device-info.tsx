      ...
      try {
        ...
        if (userData) {
          ...
          if (insertError) {
            ...
            if (insertError.code === "23505" && insertError.message.includes("user_clients_erp_user_id_key")) {
              console.log("Looks like user already has a client ID. Fetching all records to find it.");
              ...
              if (allData && allData.length > 0) {
                ...
                const matchingClient = allData.find(client => client.erp_user_id === user.id);
                if (matchingClient) {
    ...
    try {
      ...
      if (!clientId) {
        ...
        if (clientId) {
          ...
          console.log("Generated temporary IDs:", { clientId: tempClientId, userId: tempUserId });
          ...
          if (user?.id) {
            localStorage.setItem("erp_user_to_client_id_mapping", JSON.stringify({
              erp_user_id: user.id,
      ...
      const deviceData = {
        ...
        user_client_id: clientId,
        device_name: `Generated Device - ${new Date().toISOString()}`,
        device_status: "active",
        device_type: "mobile",
        protocol_type: "MQTT",
        customer_nan: "default",
        device_reg_id: `MOB-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`,
      ...
      try {
        ...
        if (!tableError) {
          ...
          console.log("user_client_devices table doesn't exist, skipping mapping creation");
        }
      } catch (error) {
