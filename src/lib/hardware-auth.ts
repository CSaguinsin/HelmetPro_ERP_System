...
function validateFallbackToken(
  token: string,
  userClientId?: string
...
export async function verifyHardwareAuth(req: NextRequest) {
  ...
  const isForRestApi = req.headers.get('x-for-rest-api') === 'true';
  ...
  console.log('Auth headers:', {
    ...
    isForRestApi: isForRestApi ? 'true' : 'false',
  });
  ...
  try {
    if (isJwtToken) {
      ...
      let userDetail: any | IJwtPayload | UserResponse = null;

      if (isForRestApi) {
        userDetail = getUser(token);
      } else {
        userDetail = await supabase.auth.getUser(token);
      }
      ...
      if (isDeviceEndpoint) {
        ...
        if (!userClientError && userClient) {
          ...
          let query = supabase.from('device_list').select('*');
      ...
      const { data: device, error: deviceError } = await supabase
        .from('device_list')
        .select('*')
        .eq('erp_client_id', userDetail.user_client_id)
        .single();
      ...
      let validation: any | IJwtPayload | UserResponse = null;

      if (isForRestApi) {
        validation = getUser(token);
      } else {
        validation = validateFallbackToken(token, userClientId?.toString());
      }

      console.log('Fallback token validation result:', validation);

      if (!validation.valid && !isForRestApi) {
        return {
          authenticated: false,
          response: NextResponse.json(
      ...
      const { data: userData, error: userError } = await supabase
        .from('users')
        .select('*')
        .eq('erp_user_id', validation.user_id)
        .single();
